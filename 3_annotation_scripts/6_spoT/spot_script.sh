...
current_dir=$(pwd)
echo "$current_dir"
python3 "$script_dir2"/1_replace_product.py "$@"
...
# Generate txt file name by replacing .fna with .txt
    ...
    blastp -query "$faa_file" -db "$script_dir2"/spot_db/aa_db/spot_aa -out "$current_dir"/"$txt_file" -outfmt 6 -num_alignments 1 -evalue 1e-30
